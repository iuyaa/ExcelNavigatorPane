...
以下命令生成 `dist/releases/1.0.13.0/渠道/ExcelNavigator-Setup-1.0.13.0.exe`。用户只需一个 EXE，无需 Visual Studio。EXE 检查桌面 Excel 已安装后，按 Windows 位数选择内置 MSI；64 位包同时注册 32/64 位宿主，共享 AnyCPU 插件；缺少 .NET Framework 4.8 或 VSTO Runtime 时由微软引导程序下载依赖。

```powershell
# 一次性准备 WiX 构建工具；用户电脑不需要 WiX
dotnet tool install wix --version 4.0.6 --tool-path work/tools/wix
./installer/Build-Installer.ps1 -Version 1.0.13.0 -UpdateChannel oneview
./installer/Build-Installer.ps1 -Version 1.0.13.0 -UpdateChannel github
```
...
```powershell
./tests/UpdatePublishCheck.ps1 -Directory dist/releases/1.0.13.0/oneview -Version 1.0.13.0 -UpdateChannel oneview
./tests/UpdatePublishCheck.ps1 -Directory dist/releases/1.0.13.0/github -Version 1.0.13.0 -UpdateChannel github
...
1.0.13.0 为内部预发布版本，完整真实升级验收尚未完成，普通修改不自动发布。用户要求“发布”时完成以下全部步骤，更新日志维护在 [CHANGELOG.md](CHANGELOG.md)：

1. 确定递增版本号并整理更新日志，同步 `Properties/UpdateSettings.xml` 和 `Properties/AssemblyInfo.cs` 的 `AssemblyFileVersion`，避免 MSI 因 DLL 文件版本未增长而跳过替换。采用 `主.次.构建.0`，最后一段必须为 0；MSI 只比较前三段（最大分别为 255、255、65535）。下一版例如 1.0.14.0。
...
python installer/Publish-RustFS.py --version 1.0.13.0 --directory dist/releases/1.0.13.0/oneview
# 用户要求发布时执行，逐文件上传并校验匿名下载，入口最后更新
python installer/Publish-RustFS.py --version 1.0.13.0 --directory dist/releases/1.0.13.0/oneview --apply
```
