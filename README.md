...
- OneDrive 云端地址按本机同步映射解析，在原同步目录保存新名称；确认工作簿已切换到新路径、保存完成且新文件落盘后删除旧文件。同名目标、只读文件、映射不明确或原文件未落盘时拒绝操作；旧文件被占用则明确提示两个路径，保留旧文件供手动清理。
- 保存或重命名成功后自动刷新工作簿列表；Excel 忙碌时延迟重试，恢复可操作后显示新名称
- 关闭明确选中的工作簿，并保留 Excel 原生的保存/取消提示

### 工作表

- 按住工作表行正文拖动，以插入线为准放到目标表前后，修改工作簿真实顺序；列表边缘自动滚动，Esc 或拖出列表松手取消，点击眼睛/锁按钮不会启动拖动。
- 普通隐藏表可拖动且保持隐藏；深度隐藏表不可拖动，编辑中或结构保护时禁止移动。移动保持当前活动表，不自动保存工作簿。
- 搜索或“仅看可见表”筛选时，按目标行在工作簿中的真实位置插入，仅移动拖动的表，不重新排列未显示的表；仅支持同一窗格、同一工作簿内拖动。
...
以下命令生成 `dist/releases/1.0.17.0/渠道/ExcelNavigator-Setup-1.0.17.0.exe`。用户只需一个 EXE，无需 Visual Studio。EXE 检查桌面 Excel 已安装后，按 Windows 位数选择内置 MSI；64 位包同时注册 32/64 位宿主，共享 AnyCPU 插件；缺少 .NET Framework 4.8 或 VSTO Runtime 时由微软引导程序下载依赖。

```powershell
# 一次性准备 WiX 构建工具；用户电脑不需要 WiX
dotnet tool install wix --version 4.0.6 --tool-path work/tools/wix
./installer/Build-Installer.ps1 -Version 1.0.17.0 -UpdateChannel oneview
./installer/Build-Installer.ps1 -Version 1.0.17.0 -UpdateChannel github
```
...
```powershell
./tests/UpdatePublishCheck.ps1 -Directory dist/releases/1.0.17.0/oneview -Version 1.0.17.0 -UpdateChannel oneview
./tests/UpdatePublishCheck.ps1 -Directory dist/releases/1.0.17.0/github -Version 1.0.17.0 -UpdateChannel github
...
1.0.17.0 为内部预发布版本，完整真实升级验收尚未完成，普通修改不自动发布。用户要求“发布”时完成以下全部步骤，更新日志维护在 [CHANGELOG.md](CHANGELOG.md)：

1. 确定递增版本号并整理更新日志，同步 `Properties/UpdateSettings.xml` 和 `Properties/AssemblyInfo.cs` 的 `AssemblyFileVersion`，避免 MSI 因 DLL 文件版本未增长而跳过替换。采用 `主.次.构建.0`，最后一段必须为 0；MSI 只比较前三段（最大分别为 255、255、65535）。下一版例如 1.0.18.0。
...
python installer/Publish-RustFS.py --version 1.0.17.0 --directory dist/releases/1.0.17.0/oneview
# 用户要求发布时执行，逐文件上传并校验匿名下载，入口最后更新
python installer/Publish-RustFS.py --version 1.0.17.0 --directory dist/releases/1.0.17.0/oneview --apply
```
...
- 工作簿重命名本质上是 Excel `SaveAs` 后清理旧文件，不覆盖已有同名目标；格式兼容和路径长度错误由 Excel 处理。OneDrive 需已同步到本机，服务端同步由 OneDrive 完成；这不是云端原位重命名，不保证保留原文件的云端版本历史与共享链接。
